...
`toRepetitiveFormConfig` (`useRepetitiveFlowConfig.ts`, pure function) maps the raw response: strips `__typename`, flattens the aliased arrays into ordered `steps`, drops `""`/`0`/empty-array echo defaults for optional inputs (`label`, `pickerQuery`, `pickerFiltersQuery`, `scopeToRelationOf.filterKey`, `maxSelection`, `overviewFields`).
...
- **`relations` (`createWhen: onCreate`)**: prefilled into the step's create form as `relationValues`, so the created entity is born linked (e.g. expression → `refWork`).
