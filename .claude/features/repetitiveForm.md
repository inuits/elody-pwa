...
- **`creatableTypeFromParentKey`** (step): when the subtype is already decided by a value on the parent entity (the one whose detail page hosts the flow, e.g. the boekenbank's `expression_type`), this names the parent metadata key to read. `RepetitiveStepField` reads `getForm(getEntityIdFromRoute()).values.intialValues[key]` (the same parent-value path `copyValueFromParent` uses), normalizes it, and matches it to one of `creatableTypes` by `entityType`. On a match it collapses the options to that single type — **no chooser is shown**; with no value or no match it falls back to the full `creatableTypes` dropdown.
