...
describe("useBreadcrumb", () => {
  let pageInfo, visitedPagesOptions, selectedVisitedPage, showVisitedPages, entityTitle, showEntityTitle, addVisitedPage, clearVisitedPages, onVisitedPageChange;

  beforeEach(() => {
    pageInfo = { value: {} };
    ({ visitedPagesOptions, selectedVisitedPage, showVisitedPages, entityTitle, showEntityTitle, addVisitedPage, clearVisitedPages, onVisitedPageChange } = useBreadcrumb(pageInfo));
  });

  test("adds visited page", () => {
    addVisitedPage({ entityTitle: "Page 1", path: "/page1" });
    expect(visitedPagesOptions.value).toEqual([{ label: "Home", value: 0 }, { label: "Page 1", value: 1 }]);
  });
