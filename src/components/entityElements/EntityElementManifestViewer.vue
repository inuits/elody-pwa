<template>
  ...
  >
    ...
    <template v-slot:content>
      ...
      />
      <div v-else><p>{{t('manifest-viewer.no-manifest')}}</p></div>
    </template>
  </entity-element-wrapper>
</template>
...
const {t} = useI18n()
const {enqueueItemForBulkProcessing, dequeueItemForBulkProcessing, isEnqueued} = useBulkOperations()
const context = BulkOperationsContextEnum.ManifestCollection
const isInManifestCollection = computed(() => isEnqueued(context, props.element.manifestUrl))
