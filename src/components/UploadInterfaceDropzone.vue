<template>
  <div class="flex flex-col">
    <div class="flex flex-col">
      <div
        v-if="infoLabelUrl"
        class="upload-info-banner flex items-center text-sm font-bold px-4 py-3 mb-2"
      >
        <svg
        ...
        <p>
          <a :href="infoLabelUrl" target="_blank" class="underline">
            {{ $t("upload-fields.link-to-documentation") }}
          </a>
        </p>
      </div>
      <div
        v-if="templateCsvs"
        class="upload-info-banner flex items-center text-sm font-bold px-4 py-3 mb-4"
        @click.stop
      >
        <svg
          class="fill-current w-4 h-4 mr-2"
        ...
        <button
          type="button"
          class="upload-info-banner__action underline pr-4 cursor-pointer"
...
<style scoped>
/* Informative strip, not a status: the accent-light tint with themed ink,
   the same pair the panel headers wear (upload.md). */
.upload-info-banner {
...
.upload-info-banner__action {
  color: inherit;
  font: inherit;
}

.upload-info-banner__action:focus-visible {
