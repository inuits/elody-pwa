<template>
  <div
    class="flex flex-row items-center justify-center bg-neutral-light p-2 gap-2 w-250 h-120 rounded-lg relative"
  >
    ...
    >
      <div v-if="visitedPagesOptions.length > 0">
        <div v-if="visitedPagesOptions.length > 0">
          <div
            v-if="visitedPagesOptions[0].label === 'Home'"
            class="flex items-center"
          >
            <unicon :name="Unicons.BookOpen.name" class="w-6 h-6 mr-2" />
          </div>
          <div
            v-else-if="visitedPagesOptions[0].label === 'Mediafiles'"
            class="flex items-center"
          >
            <unicon :name="Unicons.Image.name" class="w-6 h-6 mr-2" />
          </div>
          <div
            v-else-if="visitedPagesOptions[0].label === 'History'"
            class="flex items-center"
          >
