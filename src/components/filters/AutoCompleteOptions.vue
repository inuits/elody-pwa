...
const isAnd = computed<boolean>({
  ...
  set(newValue) {
    ...
    props.value &&
      props.value?.input.selectionInput &&
      emit(
        "update:value",
        defaultReturnMultiSelectObject(props.filter?.key, {
          value: props.value?.input.selectionInput.value,
          AndOrValue: newValue,
        })
      );
  },
...
const inputFieldMulti = computed<Maybe<Maybe<string>[]> | undefined>({
  get() {
    return props.value && props.value?.input.selectionInput
      ? props.value?.input.selectionInput.value
      : undefined;
  },
  set(value) {
    emit(
