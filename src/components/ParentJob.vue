...
export default defineComponent({
  ...
  setup(props) {
    ...
    const jobTime = computed(() => {
      const diff = new Date(props.job.end_time as string).valueOf() - new Date(props.job.start_time as string).valueOf();
      var timeElapsed = Math.ceil(diff / (1000 * 3600 * 24));
      return timeElapsed;
    });
    
    const jobStartDate = computed(() => {return new Date(props.job.start_time as string).toLocaleString();});
    const state = computed((): State => {
      switch(props.job.status){
        case Status.Finished:
          return {name: Status.Finished, color: 'green-default', message: `${jobTime.value} upload time`} as State;
        case Status.Failed:
          return {name: Status.Failed, color: 'red-default', message: 'failed'} as State;
        default:
          return {name: Status.Pending, color: 'neutral-700', message: 'pending'} as State;
      }
    });

    return { Unicons, toggleCollapse, isCollapsed, jobStartDate, jobs: [], state, jobTime };
  },
});
</script>