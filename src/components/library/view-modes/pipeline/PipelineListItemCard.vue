<template>
  <li data-cy="list-item" :class="wrapperClasses">
    <!-- no multiselect and no header row; the actions menu floats in the
         top-right corner and the card itself does not navigate -->
    ...
    <div class="w-full flex flex-col gap-1 pr-6">
      <div
        v-for="(metadataItem, idx) in readModeTeaserMetadata"
...
// The card a ListItem renders in the pipeline view mode: a fixed-width box
// with the entity's read-mode teaser metadata and its corner actions menu.
