...
import {
  updateRelationDirect,
  saveRelatedEntityData,
} from "@/composables/useUpdateRelation";
...
describe("updateRelationDirect", () => {
    ...
    const entity = makeEntity("org-1", {
      refUsers: [
        ...
        },
        {
          key: "user-2",
          editStatus: EditStatus.Unchanged,
          metadata: [{ key: "roles", value: ["member"] }],
        },
      ],
    });

    await updateRelationDirect(entity, "refUsers", "user-1", {
      roles: "editor",
    });
  ...
  it("wraps scalar in array when existing metadata entry value is an array", async () => {
    ...
    const { variables } = mockMutate.mock.calls[0][0];
    const updated = variables.formInput.relations.find(
      (r: any) => r.key === "user-1",
    );
    const rolesEntry = updated?.metadata?.find((m: any) => m.key === "roles");
  ...
  it("does not array-wrap an object value (e.g. crop coordinates) when replacing an existing entry", async () => {
    const entity = makeEntity("inscription-1", {
      refMediafiles: [
        {
          key: "mediafile-1",
          editStatus: EditStatus.Unchanged,
          metadata: [{ key: "coordinates", value: { x: 1, y: 2, w: 3, h: 4 } }],
        },
      ],
    });

    await updateRelationDirect(entity, "refMediafiles", "mediafile-1", {
      coordinates: { x: 10, y: 20, w: 30, h: 40 },
    });

    const { variables } = mockMutate.mock.calls[0][0];
    const updated = variables.formInput.relations.find(
      (r: any) => r.key === "mediafile-1",
    );
    const coordinatesEntry = updated?.metadata?.find(
      (m: any) => m.key === "coordinates",
    );
    expect(coordinatesEntry?.value).toEqual({ x: 10, y: 20, w: 30, h: 40 });
    expect(updated?.editStatus).toBe(EditStatus.Changed);
  });

  it("adds a new coordinates metadata entry (object value) when cropping for the first time", async () => {
    const entity = makeEntity("inscription-1", {
      refMediafiles: [
        { key: "mediafile-1", editStatus: EditStatus.Unchanged, metadata: [] },
      ],
    });
    ...
    expect(coordinatesEntry?.value).toEqual({ x: 1, y: 2, w: 3, h: 4 });
  });
});
...
describe("saveRelatedEntityData", () => {
  ...
  it("does nothing when form has no relatedEntityData.relations entries", async () => {
    mockGetForm.mockReturnValue({
      values: { relatedEntityData: { metadata: {}, relations: {} } },
    });
  ...
  it("skips related entities not in the provided list", async () => {
    mockGetForm.mockReturnValue({
      values: {
        relatedEntityData: {
          metadata: {},
          relations: { "roles-unknown-user": ["admin"] },
        },
      },
    });
    await saveRelatedEntityData("org-1", [makeEntity("user-1")]);
  ...
  it("unwraps formatter-wrapped values before saving into metadata array", async () => {
      ...
      refOrganizations: [
        {
          key: "org-1",
          editStatus: EditStatus.Unchanged,
          metadata: [{ key: "roles", value: ["venue_admin"] }],
        },
      ],
    });
    mockGetForm.mockReturnValue({
      values: {
        relatedEntityData: {
          metadata: {},
          relations: {
            "roles-user-1": { label: "venue_member", formatter: "pill" },
          },
        },
      },
    });
    ...
    expect(mockMutate).toHaveBeenCalledOnce();
    const { variables } = mockMutate.mock.calls[0][0];
    const updatedRelation = variables.formInput.relations.find(
      (r: any) => r.key === "org-1",
    );
    const rolesEntry = updatedRelation?.metadata?.find(
      (m: any) => m.key === "roles",
    );
