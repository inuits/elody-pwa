...
import { breadcrumbRoutes, rootRoute, useBreadcrumbs } from "@/composables/useBreadcrumbs";
import { AdvancedFilterTypes, Entitytyping, RouteNames, SearchInputType } from "@/generated-types/queries";
...
describe("useBreadcrumbs", () => {
  ...
  it("Should have correct filters when creating them", async () =>  {
    ...
    const createFiltersSpy = vi.spyOn(useBreadcrumbs(undefined), "createFilters");
  ...
  it("Should have a root route title when clearing breadcrumb path and adding overview page", async () =>  {
    ...
    const clearBreadcrumbPathAndAddOverviewPageSpy = vi.spyOn(useBreadcrumbs(undefined), "clearBreadcrumbPathAndAddOverviewPage");
    clearBreadcrumbPathAndAddOverviewPageSpy(title);

    expect(clearBreadcrumbPathAndAddOverviewPageSpy).toHaveBeenCalledTimes(1);
    expect(clearBreadcrumbPathAndAddOverviewPageSpy).toHaveBeenCalledWith("this_is_an_overviewpage test");
  ...
  it("Should have an overviewPage in the breadcrumbroutes after adding them", async () =>  {
    ...
    const addOverviewPageToBreadcrumbSpy = vi.spyOn(useBreadcrumbs(undefined), "addOverviewPageToBreadcrumb");
  ...
  it("Should get correct breadcrumbs based on config", async () =>  {
    ...
    const getRouteBreadcrumbsOfEntitySpy = vi.spyOn(useBreadcrumbs(config), "getRouteBreadcrumbsOfEntity");
    const routeBreadcrumbs = getRouteBreadcrumbsOfEntitySpy(Entitytyping.Document);
