...
3. **Resolved (verified against the live `ugent-aicap` docker environment):** `EntitiesHistory(type: Entitytyping.History, ...)` — i.e. passing the *generic* `History` sentinel — is a dead/broken path: `GetAdvancedEntities` (`modules/baseGraphql/sources/collection.ts:495-524`) special-cases `type === Entitytyping.History` into `doAdvancedHistoryEntitiesCall` (`:581`, marked `// TODO: redo or remove after the demo #139636`), which discards `advancedFilterInputs` (hardcoded `[]`) and posts to a literal `history/filter` path that **does not correspond to any registered collection-api route** (confirmed by inspecting the running `inuits-elody-ugent-aicap-collection-api-1` container's `init_api.py` and the vendored `elody` package — only `GET /history/<collection>/<id>` exists). This is why podiumnet's existing `EntitiesHistory`-based query (which correctly builds an `$entity.id`-scoped `advancedFilterInputs` filter, per `inuits-dams-docs/mkdocs/docs/setup_history.md`) doesn't actually work as scoped — the filter it builds is silently dropped server-side.
   **The working path: call `EntitiesHistory` with the *concrete* entity type instead** (e.g. `type: Entitytyping.Inscription`, not `Entitytyping.History`). `GetAdvancedEntities`'s branch condition (`if (Entitytyping.History === type)`) then takes the *other* branch — `doAdvancedEntitiesCall(type, limit, skip, advancedFilterInputs, searchValue, isHistoryCall=true, exactCount)` — which does **not** drop `advancedFilterInputs`, and appends `history=true` to the request against the entity type's own working filter route. Verified in the live `ugent-aicap` container: `inscription`'s filter route (`/entities/filter`, `ClientFilter` → `FilterGenericObjectsV2.post()`, `collection-api/api/resources/filter.py:255-258`) swaps `collection` → `config.crud()["collection_history"]` (`inscriptions_history`, confirmed in `inscription_configuration.py`) whenever `?history=` is present, and applies the given filters normally — so a proper `$entity.id`/`$entity.type` `advancedFilterInputs` scoping filter (same convention as `setup_history.md`'s `GetAssetHistoryEntitiesFilter` pattern) works correctly here. Results resolve as the concrete `Inscription` GraphQL type (not a generic history type), so the existing `fullInscription` fragment applies directly — full `entityView`/`intialValues` hydration, no new type, no new resolver.
   Remaining implementation-time task: confirm the exact relation-filter key for scoping to one aicap entity (analogous to podiumnet's `key: ["podiumnet:1|identifiers"]` — likely `aicap:1|identifiers`, to be verified against a real filter definition in the aicap client or empirically against the running container).
...
- **Version list + selected version data:** one new query document in the `ugent-aicap` client, `EntitiesHistory(type: Entitytyping.Inscription, advancedFilterInputs: [...scoped to this entity via $entity.id/$entity.type, per Finding 3...], searchInputType: AdvancedInputType, ...) { results { id ... on Inscription { ...fullInscription } } }` — reusing the existing `fullInscription` fragment, so every returned historical row is already fully hydrated (`entityView`/`intialValues`, relation labels resolved) via existing, unmodified resolvers. No backend/resolver changes needed for fetching. The dropdown is built client-side from this same result set (sort by update date, no second round trip needed) — simpler than the originally-considered two-phase thin/full fetch, since fetching "as history" no longer means paying the old resolver's hardcoded 1000-row/dropped-filter cost.
...
- **Multi-value/list relations** (`refWords`, `refMultilingualCounterparts`, today's broken `EntityListElement` case): compute the set diff (added/removed/unchanged ids) client-side from each snapshot's `relationValues[type]` — free, no query. Labels can't come from the existing `keyValue(source: relations, ...)` field for this purpose: confirmed in `formatResults` (`modules/baseGraphql/resolvers/intialValueResolver.ts`, `isMultiple` branch) that multi-value relations are flattened into one `results.join(', ')` string — there's no structured per-item `{id, label}` output to key a set-diff/chip list off. So: a **new generic, batched resolver added to `modules/baseGraphql`**, e.g. `RelationLabelsForIds(ids: [String!]!, type: String): [KeyAndLabel]`, internally reusing the existing `fetchRelationEntity` label-extraction logic per id, exposed as one batch call. Generic across every entity type (parameterized by `type`, not a duplicated schema per entity) — satisfies the "no per-entity-type duplication" constraint while avoiding N separate per-id queries. One call resolves labels for every relation field's delta (and, where needed, the full current list) on the whole page, not per field. This is the one piece of genuinely new backend/resolver logic in the whole POC — everything else (fetching, single-value relations) reuses existing resolvers unmodified.
...
- Fixing the dead `EntitiesHistory(type: History)` code path itself (Finding 3) — the new page avoids it entirely by calling with the concrete entity type instead, rather than repairing the generic sentinel path.
...
- Confirm the exact relation-filter key for scoping `EntitiesHistory(type: Entitytyping.Inscription, ...)` to one aicap entity (likely `aicap:1|identifiers`, per the podiumnet/`setup_history.md` convention) — verify against the running `ugent-aicap` docker environment.
